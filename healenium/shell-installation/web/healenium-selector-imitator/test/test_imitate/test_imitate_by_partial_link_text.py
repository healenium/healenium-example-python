...
def test_imitate_by_partial_link_text():
    user_selector = Selector(
        selector_type=SelectorType.BY_PARTIAL_LINK_TEXT, inner_text="link"
    )
    ...
    expected_result = Selector(
        ...
        inner_text="link to external page",
    )
...
def test_imitate_by_partial_link_text_not_anchor():
    user_selector = Selector(
        selector_type=SelectorType.BY_PARTIAL_LINK_TEXT, inner_text="link"
    )
...
def test_imitate_by_partial_link_text_invalid_target():
    user_selector = Selector(
        selector_type=SelectorType.BY_PARTIAL_LINK_TEXT, inner_text="link"
    )
